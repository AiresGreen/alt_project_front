  ...
  const formSchema = z.object({
    ...
    email:z.string().nonempty("L'e-mail est essentiel").email("Votre Email est non valide"),
    password: z
    .string()
    .min(8,"Le mot de passe doit avoir au moins 8 caractères")
    .refine((password) => /[A-Z]/.test(password), { message: "Le Mot de Passe doit contenir au moins une majuscule" })
    .refine((password) => /[a-z]/.test(password), { message: "Le Mot de Passe doit contenir au moins une minuscule" })
    .refine((password) => /[0-9]/.test(password), { message: "Le Mot de Passe doit contenir au moins un chiffre" })
    .refine((password) => /[!@#$%^&*()_+\-=\[\]{};':"\\|,.<>\/?~]/.test(password), { message: "Le mot de Passe doit contenir au moins un caractère spécial" }),
    passwordconfirmation: z
        .string().nonempty("Ce champ est requis")
    })
  ...
  <>
          ...
          <Form {...form}>
        <form onSubmit={form.handleSubmit(onSubmit)} className="md:space-y-8 flex flex-col p-4 md:p-12 md:gap-8 gap-8 ">
          <FormField
          ...
          <FormField
            ...
            render={({ field }) => (
              <FormItem>
                ...
                <FormControl>
                  <Input placeholder="john@malkovitch.com" {...field} />
                </FormControl>
                <FormMessage />
              </FormItem>
            )}
          ...
          <Button type="submit">Vers l'infini et au dela 
